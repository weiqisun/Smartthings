 ...
 */
metadata {
	definition (name: "RetroPie", namespace: "weiqisun", author: "Weiqi Sun") {
        capability "Polling"
        capability "Refresh"
        capability "Switch"
	}
...
// handle commands
def on() {
    message("Executing 'retro-on'")
    executeCommand("retro-on")
    sendEvent(name: "switch", value: "on", isStateChange: true)
}

def off() {
    message("Executing 'retro-off'")
    executeCommand("retro-off")
    sendEvent(name: "switch", value: "off", isStateChange: true)
}

def poll(){
	executeCommand("retro-status")
}
...
def hubActionResponse(response) {
  	...
  	def status = response.headers["retro-status"] ?: ""
	message("RetroPie status: '${status}'")
